...
class UsecaseRefinement(BaseModel):
    ...
    added: List[str] = Field(default_factory=list, description="Missing use cases that should be added")
...
class UsecaseRefinementResponse(BaseModel):
    ...
    refinements: List[UsecaseRefinement] = Field(description="List of refinements for each sentence")
...
class UsecaseFinder:
    ...
    def __init__(self, nlp_model, sents: List[str], llm: ChatOpenAI):
        self.nlp = nlp_model
        self.sents = sents
        self.llm = llm
                ...
                for subchild in child.children:
                    if subchild.dep_ == "prep" and subchild.text.lower() == "of":
                        for grandchild in subchild.children:
                            if grandchild.dep_ in {"pcomp", "pobj"} and grandchild.pos_ == "VERB":
                                return grandchild
                                      
        # Handle case: "be V_3 to [VERB]"
        has_auxpass = any(
            child.dep_ == "auxpass" and child.text == "be" 
            for child in xcomp.children
        )
        if has_auxpass:
            for child in xcomp.children:
