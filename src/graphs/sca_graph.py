...
# Model 1: GPT – dùng làm judge chính
_MODEL_CONFIG_1: Dict[str, Any] = {
    "provider": "openai",
    "name": "gpt-5",
    "temperature": 0,
...
# Model 2: DeepSeek – reasoning / judge phụ (so sánh logic, flow)
_MODEL_CONFIG_2: Dict[str, Any] = {
    "provider": "deepseek",
    "name": "deepseek-chat",
    "temperature": 0,
...
# Model 3: GPT-4.1 – full-size model, reliably returns all 3 criteria in structured output
_MODEL_CONFIG_3: Dict[str, Any] = {
    "provider": "openai",
    "name": "gpt-4.1",
    "temperature": 0,
...
Correctness is defined as the degree to which Scenario B preserves the *intended actor goal, high-level responsibility, and observable outcome* of Scenario A — even if detailed steps, flow structure, or branching differ.

This evaluation prioritizes *intent alignment and plausible interpretation*, not strict semantic identity.

--------------------------------
IMPORTANT INTERPRETATION RULE
--------------------------------

Because Scenario A may be brief, underspecified, or incomplete:

- Scenario B is allowed to reasonably infer or elaborate missing details.
- Differences in Main Flow, Alternative Flows, or Exception Flows are acceptable
  as long as they do NOT contradict Scenario A’s intent.
- Scenario B must NOT be penalized for:
  - additional steps
  - missing branches
  - merged or restructured flows
  - different causal decomposition
when the human scenario does not explicitly define them.

--------------------------------
NO-REFERENCE RULE
--------------------------------

If Scenario A (human reference) is missing or too incomplete to determine intent:

- Set all field scores to "N/A"
- Return total_score = null
- Do NOT speculate or penalize Scenario B
...
Evaluate whether Scenario B preserves the same *intended real-world actor role* as Scenario A.

20 points:
- Same external actor role
- Actor goal and responsibility are aligned at a conceptual level

10 points:
- Actor is mostly correct but slightly generalized, abstracted, or mixed with system actions

0 points:
- Different primary actor, or system/UI treated as actor

--------------------------------

2. Use Case Name (0-20 points)

Evaluate whether the use case name in Scenario B reflects the *same overall user goal* as Scenario A.

20 points:
- Same actor goal, even if phrased more generally or more concretely

10 points:
- Related goal but broader or narrower than the human version

0 points:
- Describes an internal function, UI step, or different intent

--------------------------------

3. Main Success Scenario (MSS) (0-25 points)

Evaluate whether Scenario B's MSS represents a *plausible realization* of the success intent in Scenario A.
...
25 points:
- MSS achieves the same success outcome
- Actor-system interaction is plausible and non-contradictory

18 points:
- MSS aligns with intent but simplifies, expands, or restructures logic
- Minor gaps are acceptable due to missing input

10 points:
- MSS loosely matches intent but introduces unclear causality

0 points:
- MSS contradicts the human success intent or is purely technical

--------------------------------

4. Alternative Flows (0-15 points)

Evaluate whether alternative flows represent *reasonable variations or interpretations* of the MSS.

15 points:
- Alternatives plausibly extend or vary the main success path
- Even if not explicitly stated in Scenario A

8 points:
- Alternatives exist but are weakly specified or loosely connected

5 points:
- Alternatives are missing but Scenario A does not clearly define them

0 points:
- Alternatives contradict the success goal or describe failures

--------------------------------

5. Exception Flows (0-10 points)

Evaluate whether exception flows represent *reasonable failure interpretations* given Scenario A.

10 points:
- Exception intent aligns with plausible failure conditions

5 points:
- Exception flows exist but are vague or underspecified

5 points (also acceptable):
- Exception flows are missing, but Scenario A does not define failures

0 points:
- Exception flows contradict Scenario A or are misused as success paths

--------------------------------

6. Preconditions (0-5 points)

Evaluate whether preconditions describe *reasonable required states*.

5 points:
- Preconditions are aligned with Scenario A or logically implied

3 points:
- Preconditions are vague or incomplete

0 points:
- Preconditions contradict the use case or are written as actions

--------------------------------

7. Postconditions (0-5 points)

Evaluate whether postconditions describe *observable end states* consistent with Scenario A.

5 points:
- Postconditions match the intended outcome

3 points:
- Postconditions are partial or implicit

0 points:
- Postconditions are missing or irrelevant
...
- Focus on intent alignment, not textual similarity
- Explicitly state when differences are acceptable due to missing input
- Avoid penalizing inferred but reasonable behavior
- Identify only *clear semantic contradictions* or intent mismatches

DO NOT:

- Penalize structural differences alone
- Assume Scenario A is complete
- Enforce strict step-by-step equivalence
- Evaluate design quality or implementation detail
...
def _build_evaluation_from_partial(
    ...
    """Construct a UseCaseEvaluation from the partial dict that caused the ValidationError.

    When the LLM returns only some criteria (e.g. Completeness but not
    Correctness/Relevance), Pydantic rejects the whole object.  This helper
    extracts whatever *was* returned and fills safe defaults for the rest so
    the judge result is not lost entirely.
    """

    # Pydantic v2 stores the raw input in the first error's input_value.
    raw: dict = {}
    ...
    if isinstance(comp_raw, dict):
        comp = CompletenessEvaluation(**comp_raw)
    else:
        comp = CompletenessEvaluation(
    ...
    if isinstance(corr_raw, dict):
        corr = CorrectnessEvaluation(**corr_raw)
    else:
        corr = CorrectnessEvaluation(
    ...
    if isinstance(rel_raw, dict):
        rel = SimpleCriterionEvaluation(**rel_raw)
    else:
        rel = SimpleCriterionEvaluation(
            score=0, result="FAIL", rationale="Not returned by LLM.",
            sub_scores={},
        )
...
def _judge_node(detector_name: str, *, model_key: str | None = None):
    def _fn(state: ScaState):
        ...
        def _heuristic_eval() -> dict:
            ...
            rel_sub_scores: Dict[str, int] = {
                ...
                and any(
                    ...
                    for x in (spec_obj_local.get("alternative_flows") or [])
                )
        ...
        if model is None:
            print(f"\n[JUDGE FALLBACK] {detector_name}: model is None – using heuristic.")
        ...
        try:
            ...
            # The LLM returned a partial response (e.g. only Completeness).
            # Try to salvage whatever fields were returned and fill defaults.
            import traceback
            print(f"\n[JUDGE PARTIAL] {detector_name} structured output incomplete – attempting to salvage partial result.")
            print(f"[JUDGE PARTIAL] Validation errors: {pve.error_count()}")
            try:
                evaluation = _build_evaluation_from_partial(pve, reference_spec_path)
                print(f"[JUDGE PARTIAL] {detector_name} salvaged successfully.")
            except Exception:
                traceback.print_exc()
                print(f"[JUDGE FALLBACK] {detector_name} salvage failed – using heuristic.")
                return _heuristic_eval()
        except Exception as exc:
            import traceback
            print(f"\n[JUDGE FALLBACK] {detector_name} LLM call FAILED – falling back to heuristic.")
            print(f"[JUDGE FALLBACK] Error type: {type(exc).__name__}")
            print(f"[JUDGE FALLBACK] Error message: {exc}")
            traceback.print_exc()
            return _heuristic_eval()
...
def build_sca_eval_graph():
    ...
    This graph does NOT generate or regenerate specs; it only scores what's provided
    in state['use_case_spec_json'].
    """
...
def _read_json_object_file(path_str: str) -> dict:
    """Read a JSON file and return a dict; raises on errors."""
...
def _aggregate_evaluation_from_out(out: dict) -> UseCaseEvaluation | None:
    """Aggregate the per-judge evaluations for the latest spec_version in `out`."""
...
def run_sca_use_case(
    *,
    ...
    aggregated_evaluation = _aggregate_evaluation_from_out(out)
    ...
    if isinstance(comparison_spec_path, str) and comparison_spec_path.strip():
        try:
            ...
            comparison_evaluation = _aggregate_evaluation_from_out(eval_out)
        except Exception:
